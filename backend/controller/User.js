...
export const getUser = async (req, res) => {
  try {
    const user = await User.findOne().select("-password -email"); // get user will be fetch but with out email and password.
...
export const addBlog = async (req, res) => {
  try {
    const { title, image, description, question, option1, option2, option3, option4, answer } = req.body;
...
export const addBlogQuestion = async (req, res) => {
  try {
    ...
    const existingBlog = user.blogs.find(blog => blog._id == id);
    ...
    existingBlog.questions.push(QnWithAnswer);
    ...
    res.status(200).json({
      success: true,
      message: "Question Added To the Blog",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
export const deleteBlog = async (req, res) => {
  try {
    ...
    const blog = user.blogs.find((item) => item._id == id);

    console.log("Found Blog", id, blog)
    await cloudinary.v2.uploader.destroy(blog.image.public_id);

    user.blogs = user.blogs.filter((item) => item._id != id);

    await user.save();

    res.status(200).json({
      success: true,
      message: "Deleted from Blogs",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
